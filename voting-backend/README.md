...
Для назначения последующих администраторов используется секретный код (по умолчанию `admin-secret-2024`), который передаётся в поле `adminCode` при регистрации.
